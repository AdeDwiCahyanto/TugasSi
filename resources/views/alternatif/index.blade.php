        ...
        <main>
            <div class="container-fluid px-4">
                ...
                <div class="card-body p-12 table-responsive">
                    <div class="col">
                        <a class="btn btn-primary" href="{{ route('alternatif.create') }}">Add</a>
                    </div>
                    <table class="table table-bordered table-striped table-hover mb-12">
                        ...
                        <?php $no = 1 ?>
                        @foreach($alternatif as $customer)
                        <tr>
                            <td>{{ $no++ }}</td>
                            <td>{{ $customer->nilai_alternatif}}</td>
                            <td>{{ $customer->nama_truk}}</td>
                            <td>{{ $customer->nama_supir}}</td>
                            <td>
                                <a class="btn btn-sm btn-warning" href="{{ route('alternatif.edit', $customer) }}">Edit</a>
                                <form method="POST" action="{{ route('alternatif.destroy', $customer) }}" style="display: inline-block;">
                                    @csrf
                                    @method('DELETE')
