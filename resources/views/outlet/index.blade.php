        ...
        <main>
            <div class="container-fluid px-4">
                ...
                <div class="card-body p-12 table-responsive">
                    <div class="col">
                        <a class="btn btn-primary" href="{{ route('outlet.create') }}">Add</a>
                    </div>
                    <table class="table table-bordered table-striped table-hover mb-12">
                        ...
                        <?php $no = 1 ?>
                        @foreach($outlet as $customer)
                        <tr>
                            <td>{{ $no++ }}</td>
                            <td>{{ $customer->nama_outlet}}</td>
                            <td>
                                <a class="btn btn-sm btn-warning" href="{{ route('outlet.edit', $customer) }}">Edit</a>
                                <form method="POST" action="{{ route('outlet.destroy', $customer) }}" style="display: inline-block;">
                                    @csrf
                                    @method('DELETE')
