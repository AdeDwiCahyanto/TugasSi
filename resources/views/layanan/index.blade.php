        ...
        <main>
            <div class="container-fluid px-4">
                ...
                <div class="card-body p-12 table-responsive">
                    <div class="col">
                        <a class="btn btn-primary" href="{{ route('layanan.create') }}">Add</a>
                    </div>
                    <table class="table table-bordered table-striped table-hover mb-12">
                        ...
                        <?php $no = 1 ?>
                        @foreach($layanan as $customer)
                        <tr>
                            <td>{{ $no++ }}</td>
                            <td>{{ $customer->nama_layanan}}</td>
                            <td>{{ $customer->harga_layanan}}</td>
                            <td>{{ $customer->estimasi}}</td>
                            <td>{{ $customer->ongkir}}</td>
                            <td>{{ $customer->diskon}}</td>
                            <td>
                                <a class="btn btn-sm btn-warning" href="{{ route('layanan.edit', $customer) }}">Edit</a>
                                <form method="POST" action="{{ route('layanan.destroy', $customer) }}" style="display: inline-block;">
                                    @csrf
                                    @method('DELETE')
