        ...
        <main>
            <div class="container-fluid px-4">
                ...
                <div class="card-body p-12 table-responsive">
                    <div class="col">
                        <a class="btn btn-primary" href="{{ route('pegawai.create') }}">Add</a>
                    </div>
                    <table class="table table-bordered table-striped table-hover mb-12">
                        ...
                        <?php $no = 1 ?>
                        @foreach($pegawai as $customer)
                        <tr>
                            <td>{{ $no++ }}</td>
                            <td>{{ $customer->nama_pegawai}}</td>
                            <td>{{ $customer->jenis_kelamin}}</td>
                            <td>{{ $customer->jabatan }}</td>
                            <td>{{ $customer->username }}</td>
                            <td>{{ $customer->password }}</td>
                            <td>
                                <a class="btn btn-sm btn-warning" href="{{ route('pegawai.edit', $customer) }}">Edit</a>
                                <form method="POST" action="{{ route('pegawai.destroy', $customer) }}" style="display: inline-block;">
                                    @csrf
                                    @method('DELETE')
